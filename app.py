...
async def respond(text: str, uploaded_file, generate_effects: bool) -> tuple[Path | None, str]:
    ...
    if (text_len := len(text)) > MAX_TEXT_LEN:
        gr.Warning(
            f"Input text length of {text_len} characters exceeded current limit of {MAX_TEXT_LEN} characters. Please input a shorter text."
        )
        return None, ""
...
with gr.Blocks(title="Audiobooks Generation") as ui:
    ...
    effects_generation_checkbox = gr.Checkbox(
        ...
        value=False,
        info="Select if you want to add occasional sound effects to the audiobook",
    )
    ...
    submit_button.click(
        fn=respond,
        inputs=[text_input, file_input, effects_generation_checkbox],
        outputs=[audio_output, error_output],
    )
    refresh_button.click(
        fn=refresh,
        inputs=[],
        outputs=[audio_output, error_output, file_input],
    )
...
ui.launch()  # Launch the Gradio app
